...
theme variables through the `@theme` block in `client/src/index.css`, so the
components follow the light/dark switch like the rest of the page. Tailwind
CSS v4 and Motion were added for them; the rest of the app keeps its inline
styles. Every animation honours `prefers-reduced-motion`.
